...
面向 Java/Spring 项目的 AI Agent 代码安全审计平台。用户导入 Git 仓库并选择提交后，系统以确定性代码解析提供真实事实，再由 Recon、Orchestrator、专业审计和 Report Agents 自主规划、检索上下文、建立漏洞假设并生成可复核报告。
...
- 将增量语义变化分类为方法新增、修改、删除、签名变化、Guard 新增和 Guard 删除；纯删除变更通过 Base/Target 方法正文进行识别
...
- 专业 Agent 的候选必须通过证据资格、显式行号和可执行代码门禁，系统不会把位置自动推断到其他代码行
...
- `NEED_CONTEXT` 单元补充语义影响块和直接关联的普通上下文块后复判一次
- 只有 `INVESTIGATE` 单元创建 SQL 注入、权限、敏感信息泄露、XSS 或验证绕过专业 Agent
- SQL 注入、越权、敏感信息泄露、存储 XSS、验证绕过五类独立专业 Agents
- 受控多轮工具调用，返回真实代码块而不是只有文件名
- DTO、Entity、VO、PO 后缀的 Java 类不作为独立审查目标，同时保留在索引和调用链上下文中
- 专业 Agent 提交空行号、越出主代码块或只包含注释/结构声明的位置时，本次候选会被拒绝，拒绝原因返回 Agent 供剩余迭代修正；超过 5 行的范围不会仅因长度被拒绝
- 候选通过门禁后先执行确定性去重，再由 Report Agent 生成管理摘要和覆盖说明
...
安全物化 Git 提交快照
  ...
  → 专业 Agents 多轮调用代码工具
  → 形成结构化漏洞假设
  → 专业证据和显式位置门禁，不合格候选返回原因后重试
  → 候选去重、文件和行号真实性校验
  → Report Agent 汇总报告
```

确定性规则只产生 `AuditHint` 风格的调查线索，不直接写入漏洞表。除数据承载类外，规则线索和已经形成的语义安全流属于必审单元；其他外部入口、危险操作、安全配置和增量影响代码经过轻量分流。文件名以 DTO、Entity、VO、PO 结尾的 Java 类无论是否命中规则，都不作为独立审查目标；这些类保留在代码索引中，专业 Agent 可通过只读工具将其作为调用链上下文读取。

系统覆盖全部安全相关审计单元。`triage-batch-size` 只控制每次轻量模型请求的摘要数量，不截断项目总范围；完整代码、语义工具和多轮推理只提供给 `INVESTIGATE` 单元。
...
模型地址、API Key 和模型名由环境变量或 `.env` 提供。迭代次数、工具调用次数和观察保留上限直接配置在 `application.yml`。
...
JavaParser 负责在局部 AST 中验证调用现场。CodeGraph 只决定影响范围和关系候选，不直接判定漏洞。
...
CodeGraph 是唯一的全局拓扑主来源。Base/Target 索引构建失败或增量影响范围查询失败会使任务失败；专业 Agent 作用域内的 callers/callees 关系查询按方向独立执行，某一方向失败时保留另一方向的成功结果，并回退到本地语义关系继续审查，而不是直接终止任务。`enabled=false` 仅用于使用确定性替身的自动化测试配置。
...
分析范围包含安全 XML、MyBatis Mapper、数据库迁移以及 JSP/FreeMarker/Thymeleaf 服务端模板。
...
增量报告只保留与 `CHANGED` 目标存在直接变更或语义影响关系、且能在 Target 中通过证据和位置门禁的漏洞。正式漏洞的 `deltaStatus` 为 `NEW`，稳定指纹用于同批去重和后续跨扫描匹配；报告不生成单独的 `PERSISTING` 或 `FIXED` 漏洞项。纯删除行通过 Base/Target 方法正文比较定位到 Target 方法；被删除方法保存为独立语义变化，并通过剩余调用者和同文件方法扩展影响范围。
...
应用连接空数据库时，Flyway 会执行单个 `V1__initial_schema.sql`，一次性创建项目、任务、代码块、漏洞、Agent 轨迹、Git 差异和语义分析表，以及对应索引、外键和检查约束。该初始化脚本只支持全新数据库，不支持已有数据库的原地结构升级。
...
- `read_source({chunkId,startLine,endLine,contextLines})`：读取已验证或候选代码块的真实源码；不指定行号时读取整个代码块，单次最多返回 160 行
- `verify_relation({candidateChunkId,anchorChunkId})`：验证候选与当前锚点的确定性关系
- `search_symbols({symbol,kind,annotation,filePath,endpoint,text})`：确定性检索符号和代码位置
- `search_code({query,scope,filePattern})`：在受控范围内执行源码字面量搜索
...
模型只通过结构化 `arguments` 调用工具；支持分页的搜索工具使用 `limit` 和服务端返回的 `nextCursor` 控制结果。确定性符号搜索、项目源码搜索和未映射的 CodeGraph 结果只用于发现候选，必须通过 `verify_relation` 后才能作为漏洞证据。仓库源码始终作为不可信数据传递给模型。Agent 提交的主证据和关联证据必须来自当前目标或工具返回的已验证代码块 ID，否则候选会被拒绝。增量任务还要求证据链保留当前 `CHANGED` 目标，并说明漏洞与直接变更或语义影响之间的因果关系。

专业 Agent 只保留最近 `max-detailed-observations` 条工具观察的详细正文，更早的结果按工具类型压缩进证据账本；每条详细观察最多向模型保留 `max-observation-chars` 个字符。该限制作用于模型上下文，不改变已经验证的代码块 ID 和关系状态。
...
- `GET /api/tasks/{taskId}/hypotheses`：专业 Agent 形成的漏洞假设及其当前状态
...
Agent 轨迹、漏洞和报告，但保留项目基本信息与本地裸 Git 仓库；项目恢复后可以继续扫描。
...
测试环境使用确定性的测试 LLM Gateway，并完整经过 Recon、规划、专业 Agent、工具调用、证据与位置门禁、去重和 Report 协议，不会通过关闭 AI 绕过 Agent 工作流。
...
测试使用 H2 且关闭 Flyway，不会连接或修改云端 PostgreSQL。测试类以 `IT` 结尾，普通 `mvn test` 不会自动运行它，避免意外调用付费 API；可以通过 IDEA 绿色按钮运行。
