...
Use JUnit 5, AssertJ, Spring Boot Test, MockMvc, and H2. Name automatic tests `*Test`/`*Tests`; reserve `*IT` for external checks. Cover Git source safety, commit diffing, Base/Target method matching, deletion-only hunks, signature and Guard changes, CodeGraph relation mapping, semantic resolution, evidence gates, Agent concurrency, triage decisions, `NEED_CONTEXT` enrichment, large-project coverage without fixed-target truncation, project archive/restore and cleanup cascades, task cancellation, and changed APIs. Keep default tests independent of PostgreSQL, external Git hosts, CodeGraph installations, and external models; use deterministic test doubles for external integrations.
...
Rules and semantic flows create hints, never findings, but their units are mandatory investigations so model omissions cannot silently remove them. Incremental scans must build Base/Target method snapshots, preserve method additions/modifications/deletions/signature changes and Guard additions/removals, and treat Guard removal as a mandatory investigation. Only `CHANGED` units are triage targets and may create professional tasks; related `IMPACTED` chunks are supplied as evidence after `NEED_CONTEXT` or `INVESTIGATE`, while `CONTEXT` remains available for architecture understanding and read-only tool evidence. Do not truncate semantic impact coverage with a fixed project-wide chunk count. A finding requires a professional hypothesis, valid chunk IDs, a `CHANGED` causal anchor, and Critic confirmation. CodeGraph candidates must pass local call-site verification before entering a security flow or satisfying `verify_relation`. Preserve the untrusted-code boundary and report filtering of `[SEMANTIC_FLOW]`/`[CRITIC]`.
