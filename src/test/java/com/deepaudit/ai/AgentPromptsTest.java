...
class AgentPromptsTest {
    ...
    void professionalPromptIsCompactAndKeepsTheInvestigationContract() {
        ...
        assertThat(prompt)
                 ...
                 .contains("工具不是固定流程", "每轮只调用最能补齐它的工具")
                 .contains("不得为耗尽预算遍历工具")
                 .contains("缺调用方、被调用方或可达路径时用 explore_call_graph")
                 .contains("缺输入到危险操作的传递依据时用 trace_value")
                 .contains("数据访问、参数绑定或对象/租户条件时用 resolve_data_access")
                 .contains("接口鉴权、过滤器或安全配置时用 inspect_security_policy")
                 .contains("核对 Base/Target 变化时用 get_change_context")
                 .contains("搜索结果先用 read_source 判断相关性")
                 .contains("连续工具结果没有提供新事实时，停止搜索")
